...
public class ParticleEffectList {
	...
	public List<Object> getParticleList(final String particleToSearchFor) {
		if (particleToSearchFor != null && !particleToSearchFor.isEmpty())
			if (version.atLeast(ServerVersion.Version.v1_9))
				return particleList.stream().map(Particle.class::cast).filter((effect) -> effect.name().contains(particleToSearchFor.toUpperCase())).sorted(Comparator.comparing(Enum::name)).collect(Collectors.toList());
			else
	...
	public boolean sortOut(final Effect effect) {
		return !(effect.name().equals("DUST_COLOR_TRANSITION") || effect.name().equals("LEGACY_BLOCK_CRACK")
				...
				|| effect.name().equals("MOB_APPEARANCE"));
	}
	...
	private void itemsAddedIn1_12AndOlder() {
		...
		this.particlesCached.put("ENCHANTMENT_TABLE", CreateItemUtily.of("ENCHANTMENT_TABLE").makeItemStack());
		...
		this.particlesCached.put("ITEM_CRACK", CreateItemUtily.of("NETHER_BRICK").makeItemStack());
		this.particlesCached.put("REVERSE_PORTAL", CreateItemUtily.of("ENDER_PORTAL_FRAME").makeItemStack());
		this.particlesCached.put("SNOWBALL", CreateItemUtily.of("SNOW_BALL").makeItemStack());
